...
<ol>
    ...
    <#if mode?? && mode = "manual">
        <li>
            ...
            <p><span id="sso-totp-secret-key">${totp.totpSecretEncoded}</span></p>
            <p><a href="${totp.qrUrl}" id="mode-barcode">${msg("totpScanBarcode")}</a></p>
        </li>
        <li>
            <p>${msg("totpManualStep3")}</p>
            <ul>
                <li id="sso-totp-type">${msg("totpType")}: ${msg("totp." + totp.policy.type)}</li>
                <li id="sso-totp-algorithm">${msg("totpAlgorithm")}: ${totp.policy.algorithm}</li>
                <li id="sso-totp-digits">${msg("totpDigits")}: ${totp.policy.digits}</li>
                <li id="sso-totp-period">${msg("totpInterval")}: ${totp.policy.period}</li>
            </ul>
        </li>
    <#else>
        <li>
...
<form action="${url.totpUrl}" class="form-horizontal" method="post">
    ...
    <div class="form-group">
        <div id="sso-form-buttons" class="col-md-offset-2 col-md-10 submit">
            <div class="">
